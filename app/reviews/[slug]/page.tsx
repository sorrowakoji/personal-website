...
async function getReview(slug: string) {
  const res = await fetch(`${getBaseUrl()}/api/reviews/${slug}`, {
    next: {
      revalidate: 5,
    },
  });
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
            {review.game && (
              <div className="flex items-center gap-2 text-primary mb-3">
                ...
                <span className="text-sm font-medium">{review.game.name}</span>
              </div>
            )}
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4">
                    ...
                    <dl className="space-y-3 text-sm">
                      {review.game.metadata.map((meta: any) => (
                        ...
                        >
                          <dt className="text-muted-foreground">{meta.type}</dt>

                          <dd className="text-right">{meta.value}</dd>
                        </div>
                      ))}
                    </dl>
                  </div>
