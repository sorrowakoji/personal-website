...
import { Calendar, BookOpen, Sparkles, ImageIcon, ArrowRight } from 'lucide-react'
...
export default function HomePage() {
  return (
    <main>
      ...
      <section className="relative py-24 overflow-hidden" aria-labelledby="explore-heading">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground mt-3 max-w-md mx-auto">
              Everything in one place — stream schedule, reviews, commissions, and fan love.
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {sections.map(({ href, icon: Icon, label, description, accent, accentBg, hoverBorder }) => (
              <Link
                key={href}
                href={href}
              ...
              >
                <div className={`w-11 h-11 rounded-xl border flex items-center justify-center ${accentBg}`}>
                  <Icon className={`w-5 h-5 ${accent}`} />
                </div>
                <div className="flex-1">
                  <h3 className="font-display font-bold text-foreground mb-1.5">{label}</h3>
                  <p className="text-muted-foreground text-sm leading-relaxed">{description}</p>
                </div>
                <div className={`flex items-center gap-1 text-sm font-semibold ${accent} group-hover:gap-2 transition-all duration-200`}>
